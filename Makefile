...
package-repo-bundle: $(YQ) $(YTT) $(KBLD) $(IMGPKG) prep-package-tools ## Build tar bundles for package repo with given package-values.yaml file
	cd hack/packages/package-tools && $(GO) run main.go repo-bundle generate --repository=$(PACKAGE_REPOSITORY) --registry=$(OCI_REGISTRY) --version=$(REPO_BUNDLE_VERSION) --package-values-file=$(PACKAGE_VALUES_FILE) --sub-version=$(REPO_BUNDLE_SUB_VERSION) --local-registry-url=$(LOCAL_REGISTRY_URL)
...
package-bundle-tooling:
	docker build -t package-bundle-tooling:latest \
		-f build-tooling/package-bundles/Dockerfile \
		$(if $(TOOLING_CACHE_FROM),--cache-from $(TOOLING_CACHE_FROM) --build-arg BUILDKIT_INLINE_CACHE=1,) \
		.
...
package-repo-bundle-tooling:
	docker build -t package-repo-bundle-tooling:latest \
		-f build-tooling/package-repository-bundles/Dockerfile \
		$(if $(TOOLING_CACHE_FROM),--cache-from $(TOOLING_CACHE_FROM) --build-arg BUILDKIT_INLINE_CACHE=1,) \
		.
